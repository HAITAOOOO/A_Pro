...
/**
  * @brief  恒温控制
  * @param  target_temp：期望温度 measure_temp：实际温度
  * @retval PWM占空比(比较值)
  * @usage  call in thermostatic_control() function
  */
float err,last_err;
float pout,iout,dout,out_put;
float kp ,ki, kd;
void thermostatic_control(float target_temp,float measure_temp)
{
    ...
    out_put = pout + iout + dout;
    if(out_put>=1.0f)
    {
        out_put=1.0;
    }
    else if(out_put <= 0.0f)
    {
        out_put = 0;
    }
    TIM3->CCR2=TIM3->ARR*out_put;
}
...
void imu_info()
{
    ...
    imu_ahrs_update();
    imu_attitude_update();
    if(Power_on_record&&imu.temp>58)
    {
        yaw_start=imu.yaw;
        Power_on_record--;
        HAL_Delay(1);
